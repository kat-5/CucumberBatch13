...
public class AddEmployeeSteps extends CommonMethods {
    ...
    public void user_adds_multiple_employees_from_excel_file_using_sheet_and_verify_the_employee_has_added(String sheetName) throws InterruptedException {
        ...
        while (itr.hasNext()) {
            ...
            click(emp.searchButton);
            Thread.sleep(5000);
            click(emp.IdDisplayOnTable);
            Thread.sleep(5000);
            click(emp.btnEdit);
            Thread.sleep(2000);

                Iterator<Map<String, String>> itr2 = newEmployees.iterator();
                Map<String, String> mapMoreInfo = itr2.next();
                while (itr2.hasNext()) {
                    sendText(emp.otherID, mapMoreInfo.get("OtherId"));
                    sendText(emp.SSN, mapMoreInfo.get("SSN"));
                    sendText(emp.DriverLicenseNum, mapMoreInfo.get("DriverLicenseNum"));
                    emp.DriverLicenseExp.clear();
                    sendText(emp.DriverLicenseExp, mapMoreInfo.get("DriverLicenseExp"));
                    sendText(emp.SINNumber, mapMoreInfo.get("SINNumber"));


                    if (mapMoreInfo.get("Gender").equals("Female")) {
                        click(emp.Female);
                    } else {
                        click(emp.Male);
                    }

                    Select select = new Select(emp.MaritalStatus);

                    select.selectByVisibleText(mapMoreInfo.get("MaritalStatus"));


                    Thread.sleep(2000);


                    select = new Select(emp.Nationality);
                    select.selectByVisibleText(mapMoreInfo.get("Nationality"));
                    ...
                    emp.DOB.clear();
                    Thread.sleep(2000);
                    sendText(emp.DOB, mapMoreInfo.get("DOB"));
                    Thread.sleep(2000);


                        if(mapMoreInfo.get("Smoker").equals("yes")) {

                            click(emp.Smoker);
                        }
                        else{break;}

                    sendText(emp.NickName, mapMoreInfo.get("Nickname"));
                    sendText(emp.MilitaryService, mapMoreInfo.get("MilitaryService"));
